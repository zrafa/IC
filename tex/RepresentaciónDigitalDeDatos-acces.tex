...
número \(n\) está escrito en base 2, \textbf{con \(k\) dígitos},
entonces \[n = x_{k-1}\times 2^{k-1} + ... + x_1\times2^1+x_0\times2^0\]
y, si queremos escribir el más grande de todos, deberán ser todos los
\(x_i\) iguales a 1. (\textbf{Pregunta}: ¿Por qué si el número \(n\)
tiene \(k\) dígitos binarios, el índice del más significativo es \(k-1\)
y no \(k\)?)
...
\protect\hyperlink{Suma2k-1}{¿Por qué esto es cierto?}

Usando ambos argumentos hemos llegado a que el número más grande que
podemos representar con \(k\) dígitos binarios es \(2^k-1\). Por lo
tanto, \textbf{el rango de representación de un sistema sin signo a
\(k\) dígitos, o SS(k), es \([0, 2^k - 1]\)}. Todos los números
representables en esta clase de sistemas son \textbf{positivos o cero}.
...
original \(a\), y por lo tanto tiene la propiedad de que \(a\) y
C2(\(a\)) suman 0:
...
\tightlist
\item
  Busquemos el complemento a 2 de \(111010\). Invirtiendo todos los
  bits, obtenemos \(000101\). Sumando 1, queda \(000110\).
\item
...
\protect\hyperlink{ComplementoALaBase}{¿Por qué funciona este método?}

\hypertarget{representaciuxf3n-en-complemento-a-2}{%
